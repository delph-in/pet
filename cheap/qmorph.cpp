...
#define LETTERSET_CHAR ((QChar) '\x8')
...
void morph_letterset::set(const string& name, const string& elems_u8) {
  _name = name;
  QString elems = Conv->convert(elems_u8);
...
void morph_letterset::print(std::ostream &out) const {
  out << "!" << _name << " -> ";
  for(std::set<QChar>::const_iterator it = _elems.begin();
      it != _elems.end(); ++it)
      out << (Conv->convert(*it)).toUtf8().data();
  out << " [" << (Conv->convert(_bound)).toUtf8().data() << "]";
}
...
bool morph_subrule::establish_and_check_bindings(const QString& matched)
{
  ...
  while(it1.hasNext())
  {
    ...
    if(c1 == LETTERSET_CHAR)
    {
      ...
      // initiator
      c1 = it1.next32PostInc();
      morph_letterset *ls = _analyzer->letterset(string(1, c1.toLatin1())); // TODO check
      if(ls == 0)
        throw tError("Referencing undefined letterset !" + string(1, c1.toLatin1())); // TODO check
 ...
 */
bool morph_subrule::base_form(const QString& matched,
                              const QString& rest,
                              QString &result)
{
  ...
  while(it.hasNext())
  {
    ...
    {
      c = it.next32PostInc();
      morph_letterset *ls = _analyzer->letterset(string(1, c.toLatin1())); // TODO check
      if(ls == 0)
        throw tError("Referencing undefined letterset !" + string(1, c.toLatin1())); // TODO check
...
void trie_node::add_path(const QString& path, morph_subrule *rule,
                         const morph_lettersets &lettersets)
{
  ...
  else
  {
    ...
    else
    {
      ...
      string lsname = string(1, c.toLatin1()); // TODO check
      morph_letterset *ls = lettersets.get(lsname);

      if(ls == 0)
        throw tError("Referencing undefined letterset !" + lsname);

      const set<QChar> &elems = ls->elems();
      for(set<QChar>::const_iterator it = elems.begin();
          it != elems.end(); ++it) {
...
void trie_node::print(std::ostream &out, int depth) const {
  ...
  for(tn_const_iterator it = _s.begin(); it != _s.end(); ++it) {
    out << std::setw(depth) << "" 
        << "[" << (Conv->convert(it->first)).toUtf8().data() << "]" << endl;
...
void reverse_subrule(QString &s)
  // basically just a reverse, but restoring !-sequences 
{
  std::reverse(s.begin(), s.end());
  
  int off = 0;
  while((off = s.indexOf((QChar) LETTERSET_CHAR, off)) != -1) {
      std::reverse(s.begin()+off-1, s.begin()+off+1); // TODO check
  }
}
...
void morph_trie::add_subrule(grammar_rule *rule, string subrule)
{
  ...
  QString left = Conv->convert(left_u8);
  QString right = Conv->convert(right_u8);
...
list<tMorphAnalysis> morph_trie::analyze(tMorphAnalysis analysis) {
  ...
  QString s = Conv->convert(analysis.base());
  if(_suffix) reverse(s.begin(), s.end());
  ...
  while(s.length() > 0)
  {
        ...
        r != node->rules().end(); ++r)
    {
      ...
      if(_suffix) reverse(base.begin(), base.end());
...
tMorphAnalyzer::tMorphAnalyzer()
  ...
  string foo = cheap_settings->value("orthographemics-maximum-chain-depth");
  if(!foo.empty())
      _maximal_depth = boost::lexical_cast<int>(foo);
  foo = cheap_settings->value("orthographemics-minimum-stem-length");
  if(!foo.empty())
      _minimal_stem_length = boost::lexical_cast<int>(foo);
