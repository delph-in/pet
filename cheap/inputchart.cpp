...
discount_covered_items(list<class lex_item *> &lex_items)
{
    ...
    char *v;
    if((v = cheap_settings->value("lexical-covering-discount")) != 0)
        discount = strtoint(v, "as value of lexical-covering-discount");
    
    if(discount == 0)
        return;
    
    for(list<lex_item *>::iterator lex_it1 = lex_items.begin();
        lex_it1 != lex_items.end(); ++lex_it1)
    {
        for(list<lex_item *>::iterator lex_it2 = lex_items.begin();
            lex_it2 != lex_items.end(); ++lex_it2)
        {
            if(lex_it1 == lex_it2)
                continue;
            
            if((*lex_it1)->start() >= (*lex_it2)->start() &&
               (*lex_it1)->end() <= (*lex_it2)->end() &&
               (*lex_it1)->span() < (*lex_it2)->span())
            {
                if(verbosity > 4)
                {
                    fprintf(ferr, "\nAdjusting ");
                    (*lex_it1)->print(ferr);
                    fprintf(ferr, " which is covered by ");
                    (*lex_it2)->print(ferr);
                }
