...
bool passive_item_exists(chart *C, int start, int end, type_t type, list<tItem*> dtrs)
{
...
tItem* build_pcfg_combined_item(chart *C, tItem *active, tItem *passive)
{
...
void parse_finish_pcfg(fs_alloc_state &FSAS, list<tError> &errors)
{
  ...
  if (resources_exhausted(pedgelimit, memlimit, timeout)) {
    ...
    if (memlimit > 0 && t_alloc.max_usage() >= memlimit)
      ...
      s << "timed out (" << get_opt_int("opt_timeout") << " s)";
