...
fs::fs(const string& path, type_t type)
{
  if(! is_type(type)) {
    throw tError("construction of non-existent dag requested");
  }
...
fs fs::get_attr_value(int attr) const
{
  return fs(dag_get_attr_value(_dag, attr));
}

fs fs::get_attr_value(const string& attr) const
{
...
fs fs::get_path_value(const list_int *path) const
{
  return fs(dag_get_path_value(_dag, const_cast<list_int*>(path)));
}

fs fs::get_path_value(const char *path) const
{
