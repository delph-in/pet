...
initialize_version()
{
  ...
  const char *cqcsu = cheap_settings->value("qc-structure-unif");
  string qcsu = cqcsu ? qcsu : "";
  const char *cqcss = cheap_settings->value("qc-structure-subs");
  string qcss = cqcss ? cqcss : "";
  ...
  CHEAP_VERSION = string("PET(")
    + da
    + " cheap v" + version_string
    + ") [" + lexical_cast<string>(get_opt_int("opt_pedgelimit"))
    + "] " + btc(0 != packing)
    + "PA(" + lexical_cast<string>(packing)
    + ") " + btc(NULL != Grammar->sm())
    + "SM(" + ((NULL != Grammar->sm()) ? Grammar->sm()->description() : "")
    + ") RI[" + keypos[max(get_opt_int("opt_key"), 4)]
  + "] " + botc("opt_hyper")
    + "HA(" + lexical_cast<string>(Grammar->nhyperrules())
    + ") " + botc("opt_filter")
    + "FI " + iotc("opt_nqc_unif")
    + "QCU[" + lexical_cast<string>(get_opt_int("opt_nqc_unif"))
    + "(" + qcsu
    + ")] " + iotc("opt_nqc_subs")
    + "QCS[" + lexical_cast<string>(get_opt_int("opt_nqc_subs"))
    + "(" + qcss
    + ")] " + iotc("opt_nsolutions")
    + "OS[" + lexical_cast<string>(get_opt_int("opt_nsolutions"))
    + "] " + botc("opt_shrink_mem")
    + "SM " + botc("opt_shaping")
    + "SH {ns " + lexical_cast<string>(sizeof(dag_node))
    + "} (" + __DATE__
    + "/" + __TIME__
    + ") <" + sts
    + ">";
  ...
  CHEAP_PLATFORM = "gcc " + lexical_cast<string>(__GNUC__) + "." + lexical_cast<string(__GNUC_MINOR__);
#if defined(__OPTIMIZED__)
  CHEAP_PLATFORM += " (optim)";
#endif
#else
  CHEAP_PLATFORM = "unknown";
#endif
}
...
{
  capi_printf("(:application . \"%s\") ", CHEAP_VERSION.c_str());
  capi_printf("(:platform . \"%s\") ", CHEAP_PLATFORM.c_str());
