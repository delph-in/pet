...
tMEM::parseModel()
{
    ...
    */
    ...
    match_keyword("begin");
    match(T_COLON, "`:' before keyword", true);
    string tmp = match(T_ID, "mem|model", false);
    if(tmp == "mem") 
      _format = 0;
    else if (tmp == "model")
      _format = 1;
...
void tPCFG::parseModel()
{
    match(T_COLON, "`:' before keyword", true);
    match_keyword("begin");
    match(T_COLON, "`:' before keyword", true);
    string tmp = match(T_ID, "pcfg", false);
    if (tmp != "pcfg") {
    ...
    int nFeatures = lexical_cast<int>(tmp); // as number of rules in PCFG
    match(T_DOT, "`.' after section opening", true);
    parseFeatures(nFeatures);
...
tPCFG::parseOptions() {
    ...
    while(LA(0)->tag != T_EOF) {
        ...
        if (LA(0)->tag == T_ID) {
            pname = match(T_ID, "parameter name", false);
            if (pname == "*pcfg-use-preterminal-types-p*") {
                match(T_ISEQ, "iseq after parameter name", true);
                if (LA(0)->tag == T_COLON) consume(1);
                pvalue = match(T_ID, "parameter value", false);
                match(T_DOT, "dot after parameter value", true);
                _use_preterminal_types = (pvalue =="yes");
            }
            else if (pname == "*pcfg-include-leafs-p*") {
                match(T_ISEQ, "iseq after parameter name", true);
                if (LA(0)->tag == T_COLON) consume(1);
                pvalue = match(T_ID, "parameter value", false);
                match(T_DOT, "dot after parameter value", true);
                _include_leafs = (pvalue == "yes");
            }
            else if (pname == "*pcfg-laplace-smoothing-p*") {
                match(T_ISEQ, "iseq after parameter name", true);
                if (LA(0)->tag == T_COLON) consume(1);
                pvalue = match(T_ID, "parameter value", false);
                match(T_DOT, "dot after parameter value", true);
                _laplace_smoothing = lexical_cast<double>(pvalue);
            }
            else {
                while(LA(0)->tag != T_DOT) consume(1);
                consume(1);
...
void tPCFG::parseFeature(int n)
{
    ...
    int lhsfreq = lexical_cast<int>(tmp);
    tmp = match(T_ID, "rule frequency", false);
    int rulefreq = lexical_cast<int>(tmp);
...
void tGM::parseModel()
{
    ...
    tmp = match(T_ID, "number of rules", false);
    int nFeatures = strtoint(tmp.c_str(), "as number of rules in PCFG");
    match(T_DOT, "`.' after section opening", true);
