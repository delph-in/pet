...
class tReppTokenizer : public tTokenizer {
   ...
   public:
      tReppTokenizer(const std::string path, bool verbose=false);
      ~tReppTokenizer();
      ...
      //allow runtime updating of maintokenizer and calls
      void updateReppTokenizer(std::string &input);

   private:

      bool _verbose;
      std::string _format; //output format
      std::string _maintokenizer; //main tokenizer module
      std::set<std::string> _calls; //active modules
...
class tRepp {
   ...
   public:
      tRepp(std::string name, tReppTokenizer *parent, bool verbose=false);
      ~tRepp();
